;(function($) {
	...
	$.fn.extend({
				takungaeImgup : function(opt, serverCallBack) {
					...
					$(this).change(function() {
						...
						if (fileList.length > imageNum|| totalNum > imageNum) {
							...
							for ( var i = 0; i < fileList.length; i++) {
								...
								$img0.attr("src",location.protocol+'//'+document.domain+"/upload_imgs/img/a7.png").appendTo($section);
								var $img = $("<img class='up-img up-opcity'>");
								$img.attr("src", imgArr[i]);
								$img.appendTo($section);
								var $p = $("<p class='img-name-p'>");
								$p.html(fileList[i].name).appendTo($section);
                                var $input = $("<input id='taglocation ' class='taglocation' name='taglocation[]' value='' type='hidden'>");
						        $input.appendTo($section);
						        var $input2 = $("<input id='tags' name='tags' value='' type='hidden'/>");
						        $input2.appendTo($section);
								uploadImg(opt, fileList[i],$section,$input);
							}
							;
						}
					...
					function uploadImg(opt, file, obj,inpute) {
						$("#imguploadFinish").val(false);
						// 验证通过图片异步上传
						var url = opt.url;
						var data = new FormData();
						// console.log(opt.formData);
						for(var params in opt.formData){
                            data.append(""+params,opt.formData[""+params]);
							...
							success : function(data) {
								...
								if (data.success) {
									...
									var htmlStr = "<input type='text' style='display:none;' name='"
											...
											+ "'>";
									obj.append(htmlStr);
                                    inpute.val(data.path+data.name+"."+data.entension);
                                    if (successCallBack) {
