    ...
    <script>
        ...
        function del(id) {
            layer.confirm('确定删除吗?', {
                ...
                $.post("{{url('merchant/delrole')}}", {_token: "{{csrf_token()}}", id: id },
                    function (data) {
                        if (data.success) {
        ...
        function addrole() {
            $.post("{{url('merchant/addrole')}}", {_token: "{{csrf_token()}}",
                    name: $('#name').val(),
        ...
        function getpermission(id) {
            $.post("{{url('merchant/getpermission')}}", {_token: "{{csrf_token()}}",
                    id: id
                },
                function (data) {
                    if (data.success) {
                        ...
                        for(var i =0;i < rolepermission.length;i++){
                            permission_idArr.push(rolepermission[i].merchant_permission_id)
                        }
                        //获得数据后开始渲染视图
...
//                             			console.log(value.id)
            ...
            $('#sure_submit').click(function () {
                ...
                $.post("{{url('merchant/setrolepermission')}}", {_token: "{{csrf_token()}}",
                        role_id: id ,
                        id:iDarr
                    },
                    function (data) {
